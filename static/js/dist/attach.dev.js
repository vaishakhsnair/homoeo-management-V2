...
function setviewer(event) {
  ...
  for (i = 0; i < filelist.length; i++) {
    ...
    innerdiv.file = file;
    innerdiv.innerHTML = "".concat(i + 1, ". ").concat(file.name, "  <button type='button' class='viewbt'>View</button>");
...
function getAttachData(PatientNum) {
  //var stuff = {}
  var xhr = new XMLHttpRequest();
  xhr.open("GET", "/api/attachments?pid=".concat(PatientNum));
...
function setExist(PatientNum, filelist) {
  ...
  for (i = 0; i < filelist.length; i++) {
    ...
    console.log(file);
    var innerdiv = document.createElement('div');
    innerdiv.filelink = "/api/attachments?pid=".concat(PatientNum, "&files=").concat(file);
    innerdiv.innerHTML = "".concat(i + 1, ". ").concat(file, "  <button type='button' class='viewbt'>View</button>");
