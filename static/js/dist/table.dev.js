...
var rowlen = ['serial', 'container', 'medium', 'meds', 'dos', 'cost', 'multiply', 'remove', 'duplicate'];
var costtab = {};
var finalsum = {};
var totalrowcount = 0;
var consultation = 100;
var AUTOFILLENDPOINT = '/api/autofill';

function addrow() {
  rowid = "row".concat(totalrowcount);
  table = document.getElementById('table');
  var currlen = table.children[0].children.length;
  currrow = table.insertRow(currlen);
  currrow.setAttribute('id', "row".concat(totalrowcount));
  currrow.setAttribute('class', "tablerow");
  costtab[rowid] = [];

  for (i = 0; i < rowlen.length; i++) {
    currcell = currrow.insertCell(i);
    cellid = "".concat(rowlen[i], "of").concat(totalrowcount);
    cellname = rowlen[i];
    cellnum = i;
    console.log(cellid);
    ...
    switch (cellnum) {
      ...
      case 3:
        ...
        inputdiv.setAttribute('id', "input".concat(cellid));
        inputdiv.setAttribute('class', 'medsinputdiv');
        inputdiv.setAttribute('onclick', "handlemedsclick('input".concat(cellid, "','").concat(cellid, "')"));
      ...
      case 5:
        currcell.setAttribute('onclick', "costcalc('".concat(rowid, "')"));
        break;
      ...
      case 7:
        ...
        removerowbt.setAttribute('onclick', "removerow('row".concat(totalrowcount, "')"));
      ...
      case 8:
        ...
        duplicaterowbt.setAttribute('onclick', "duplicaterow('row".concat(totalrowcount, "')"));
      ...
      default:
        ...
        currcell.setAttribute('onclick', "handleclick(".concat(cellnum, ",'").concat(cellid, "')")); // currcell.appendChild(input)
...
function handleclick(cellnum, cellid) {
  ...
  input = document.getElementById("input".concat(cellid));
  ...
  cell.addEventListener('keydown', function (event) {
    if (event.keyCode == 13 && cellnum !== rowlen.length - 1 && cellnum !== 3) {
      // only fires if enter/not last cell or if meds cell
      event.preventDefault(); //cancels enter key

      currlen = table.children[0].children.length - 1;
      console.log('cellnum :', rowlen[cellnum]);
      newcellid = "".concat(rowlen[cellnum + 1]).concat(currlen);
  ...
  cell.addEventListener('input', function (event) {
    ...
    console.log(content);
    xhrobj = sendpost(AUTOFILLENDPOINT, {
      query: content,
      asker: rowlen[cellnum]
    });
...
function createautofill(cellid, data) {
  var clickevent = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'setautofill';
  removeautofill();
  cell = document.getElementById(cellid);
  ...
  for (i = 0; i < data.length; i++) {
    ...
    entry.setAttribute('onclick', "".concat(clickevent, "('").concat(cellid, "','").concat(nowlist.join(' '), "',").concat(serial, ")"));
...
function costcalc(rowid) {
  ...
  for (var i = 1; i <= 3; i++) {
    switch (i) {
      case 3:
        ...
        try {
          for (var _iterator = _meds[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            each = _step.value;
  ...
  xhrresp.onload = function () {
    ...
    for (var _i = 0, _Object$keys = Object.keys(costtab); _i < _Object$keys.length; _i++) {
      each = _Object$keys[_i];
...
function setmeds(cellid, value, serial) {
  ...
  innerdiv.setAttribute('id', "".concat(cell.children.length).concat(cellid));
  ...
  removebutton.setAttribute('onclick', "removemedsentry('".concat(cell.children.length).concat(cellid, "')"));
...
function removemedsentry(innerdivid) {
  ...
  var rowid = innerdiv.parentElement.parentElement.id;
    ...
    for (var i = 1; i <= 6; i++) {
      switch (i) {
        case 3:
          ...
          try {
            for (var _iterator2 = _meds2[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
              each = _step2.value;
