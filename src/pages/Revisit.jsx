...
export default function Revisit() {
    ...
    const [prevData, setPrevData] = useState(rowData);
    
    useEffect(() => {

        const urlParams = new URLSearchParams(window.location.search);
        const patientnum = urlParams.get('patientno');
        axios.post('/api/v2/getpatientdetails', {patientno:patientnum})
            .then(res => {
                console.log(res)
                if(res.data.status === 'success'){
                    console.log(res.data.data)
                    const patientDataFromServer = JSON.parse(res.data.data[4]);
                    console.log(patientDataFromServer)
                    setDetails(patientDataFromServer.basicDetails);
                    setComplaints(patientDataFromServer.presentingComplaints);
                    setHistory(patientDataFromServer.patientHistory);
                    setExamination(patientDataFromServer.regionalExamination);
                    setRepertorisation(patientDataFromServer.repertory);
                    setMisc(patientDataFromServer.miscellaneous);
                    setPrevData(patientDataFromServer.meds)
