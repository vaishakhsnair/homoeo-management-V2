...
function inputwatcher(input, autofill, optionclassname) {
  ...
  inputspace.addEventListener("keyup", function (e) {
    ...
    if ([37, 38, 39, 40].includes(e.keyCode) && childrenoptions.length > 0) {
      switch (e.keyCode) {
        case 40:
          ...
          if (currfocus === null) {
            currfocus = 0;
            childrenoptions[currfocus].setAttribute("id", "".concat(optionclassname, "activeautofill"));
          } else {
    ...
    if (inputspace.value.length > 0 && ![37, 38, 39, 40].includes(e.keyCode)) {
      ...
      xhrobj = sendpost("/autofill", {
        query: inputspace.value
      });
      ...
      xhrobj.onload = function () {
        ...
        for (i = 0; i < resp.length; i++) {
          autofillspace.innerHTML += "<button class=\"".concat(optionclassname, "autofilloptions\" value = '").concat(resp[i], "'>").concat(resp[i], "</button>");
        }
